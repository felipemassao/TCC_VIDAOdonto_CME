...
function initStereoCamera () {
	...
	var horizontalFOV : float = (2 * Mathf.Atan(Mathf.Tan((GetComponent.<Camera>().fieldOfView * Mathf.Deg2Rad) / 2) * GetComponent.<Camera>().aspect))*Mathf.Rad2Deg;
	...
	if (useStereoShader) {
		...
		*/
		...
		if (format3D != mode3D.SceneScreens) {
			...
			leftCam.GetComponent.<Camera>().rect = Rect(0,0,1,1);
			rightCam.GetComponent.<Camera>().rect = Rect(0,0,1,1);
			leftCam.GetComponent.<Camera>().targetTexture = sceneScreenL;
			rightCam.GetComponent.<Camera>().targetTexture = sceneScreenR;
		}
	}
